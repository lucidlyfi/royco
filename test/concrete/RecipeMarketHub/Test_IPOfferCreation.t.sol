...
contract Test_IPOfferCreation_RecipeMarketHub is RecipeMarketHubTestBase {
    ...
    function test_CreateIPGdaOffer_ForTokens() external prankModifier(ALICE_ADDRESS) {
        bytes32 marketHash = createMarket();
        ...
        // Expect the IPOfferCreated event to be emitted
        vm.expectEmit(false, false, false, false, address(recipeMarketHub));
        emit RecipeMarketHubBase.IPGdaOfferCreated(
            0, // Expected offer ID (starts at 0)
            bytes32(0),
            marketHash, // MarketID
            quantity, // Total quantity
        ...
        // Create the IP offer
        bytes32 offerHash = recipeMarketHub.createIPGdaOffer(
            marketHash, // Referencing the created market
            quantity, // Total input token amount
            expiry, // Expiry time
            tokensOffered, // Incentive tokens offered
            incentiveAmountsOffered, // Incentive amounts offered
            gdaParams // GDA math params
        );

        // Assertions on the offer
        assertEq(recipeMarketHub.numIPGdaOffers(), 1); // IP offer count should be 1
        assertEq(recipeMarketHub.numAPOffers(), 0); // AP offers should remain 0

        // Use the helper function to retrieve values from storage
        uint256 frontendFeeStored = recipeMarketHub.getIncentiveToFrontendFeeAmountForIPGdaOffer(offerHash, tokensOffered[0]);
        uint256 protocolFeeAmountStored = recipeMarketHub.getIncentiveToProtocolFeeAmountForIPGdaOffer(offerHash, tokensOffered[0]);
        uint256 incentiveAmountStored = recipeMarketHub.getMaxIncentiveAmountsOfferedForIPGdaOffer(offerHash, tokensOffered[0]);
